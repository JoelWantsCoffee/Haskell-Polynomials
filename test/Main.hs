...
assertFactoring :: forall r. (Show r, GCDD r, UFD (Polynomial r)) => (Polynomial r) -> [Polynomial r] -> Assertion
assertFactoring p lst = do (fmap expand . listify . factor $ p) @?= (List.reverse $ List.sort $ expand <$> lst)
